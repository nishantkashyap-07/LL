      ...
      <div className="space-y-6">
        {compareList.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-neutral-100 mb-2">No vehicles to compare</h3>
            <p className="text-neutral-500 text-sm">Add vehicles to start comparing their features.</p>
          </div>
        ) : (
          <>
            {/* Vehicle Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {compareList.map((vehicle, index) => (
                <motion.div key={vehicle.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}
                  ...
                  <h3 className="font-semibold text-neutral-100 mb-1">{vehicle.name}</h3>
                  <div className="text-xl font-bold text-neutral-100 mb-2">
                    ₹{vehicle.price}<span className="text-xs font-normal text-neutral-500 ml-1">/ day</span>
                  </div>
              ...
              {compareList.length < 3 && (
                <div className="bg-neutral-900 border-2 border-dashed border-neutral-700 rounded-2xl flex items-center justify-center min-h-[180px]">
                  <div className="text-center">
                    <Plus className="w-7 h-7 text-neutral-600 mx-auto mb-2" />
                    <p className="text-neutral-500 text-sm">Add Vehicle</p>
            ...
            {compareList.length > 1 && (
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden">
                <div className="p-5 border-b border-neutral-800">
                  <h3 className="text-base font-semibold text-neutral-100">Feature Comparison</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="bg-neutral-800/50 border-b border-neutral-800">
                        <th className="text-left py-3 px-5 text-xs font-medium text-neutral-400">Feature</th>
                        {compareList.map((v) => (
                          <th key={v.id} className="text-left py-3 px-5 text-xs font-medium text-neutral-300">{v.name}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-800">
                      {comparisonFeatures.map((feature) => (
                        <tr key={feature.key}>
                          <td className="py-3 px-5 text-xs font-medium text-neutral-400">{feature.label}</td>
                          {compareList.map((v) => (
                            <td key={v.id} className="py-3 px-5 text-sm text-neutral-300">
            ...
            {compareList.length > 1 && (
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
                <h3 className="text-base font-semibold text-neutral-100 mb-4">Features</h3>
                <div className="space-y-4">
                  {compareList.map((vehicle) => (
                    <div key={vehicle.id}>
                      <h4 className="text-sm font-medium text-neutral-300 mb-2">{vehicle.name}</h4>
                      <div className="flex flex-wrap gap-2">
                        {vehicle.features?.map((f, idx) => (
                          <span key={idx} className="px-3 py-1 bg-neutral-800 border border-neutral-700 text-neutral-300 text-xs rounded-full">{f}</span>
                        ))}
                      </div>
                    </div>
                  ))}
