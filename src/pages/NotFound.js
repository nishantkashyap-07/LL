      ...
      <div className="max-w-lg w-full text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="space-y-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-neutral-100 mb-3">Page Not Found</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button onClick={() => window.history.back()} className="inline-flex items-center justify-center gap-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-medium px-6 py-3 rounded-xl transition-all border border-neutral-700 text-sm">
                <ArrowLeft className="w-4 h-4" />Go Back
              </button>
            </div>

            <div className="pt-6 border-t border-neutral-800">
              <p className="text-sm text-neutral-500 mb-4">Or try these pages:</p>
              <div className="flex flex-wrap gap-2 justify-center">
                {[
                  ...
                  <Link key={item.to} to={item.to} className="px-4 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 rounded-lg text-sm transition-colors border border-neutral-700">
                    {item.label}
                  </Link>
                ))}
              </div>
