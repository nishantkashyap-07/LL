...
const Help = () => {
  const helpCategories = [
    ...
    { icon: Settings, title: 'Technical Support', description: 'App issues, bugs, and technical assistance', link: '/contact', color: 'text-neutral-300', bg: 'bg-neutral-700/50' }
  ];
  ...
  return (
    <div className="pt-20 min-h-screen bg-neutral-950">
      ...
      {/* Header */}
      <div className="bg-neutral-950 border-b border-neutral-800/50 py-16">
        <div className="container-elegant text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-neutral-400 max-w-xl mx-auto">Find answers, guides, and get support for all your questions</p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="container-elegant py-16">
        <h2 className="text-xl font-semibold text-neutral-100 mb-6">Browse by Category</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {helpCategories.map((cat, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <Link to={cat.link} className="block bg-neutral-900 border border-neutral-800 hover:border-neutral-600 rounded-2xl p-6 transition-all group">
                <div className={`w-11 h-11 ${cat.bg} rounded-xl flex items-center justify-center mb-4`}>
                  <cat.icon className={`w-5 h-5 ${cat.color}`} />
                </div>
                <h3 className="text-base font-semibold text-neutral-100 mb-1 group-hover:text-white">{cat.title}</h3>
      ...
      {/* Popular Topics */}
      <div className="border-t border-neutral-800/50 bg-neutral-900/30">
        <div className="container-elegant py-16">
          <h2 className="text-xl font-semibold text-neutral-100 mb-6">Popular Topics</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {popularTopics.map((topic, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }}>
                <Link to="/faq"
                  className="block p-4 bg-neutral-900 border border-neutral-800 rounded-xl hover:border-neutral-600 transition-colors text-sm text-neutral-300 hover:text-neutral-100">
                  {topic}
                </Link>
              </motion.div>
            ))}
      ...
      {/* Quick Links */}
      <div className="container-elegant py-16">
        <h2 className="text-xl font-semibold text-neutral-100 mb-6">Quick Links</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickLinks.map((link, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
              <Link to={link.link} className="block bg-neutral-900 border border-neutral-800 hover:border-neutral-600 rounded-2xl p-6 text-center transition-all group">
                <div className="w-11 h-11 bg-neutral-800 rounded-xl flex items-center justify-center mx-auto mb-4">
                  <link.icon className="w-5 h-5 text-neutral-300" />
                </div>
                <h3 className="font-semibold text-neutral-100 mb-1 group-hover:text-white text-sm">{link.title}</h3>
            ...
            {contactMethods.map((method, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 text-center">
                  ...
                  <h3 className="font-semibold text-neutral-100 mb-1 text-sm">{method.title}</h3>
