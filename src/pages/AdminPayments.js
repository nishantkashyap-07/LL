      ...
      <div className="container-elegant py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-neutral-500 text-sm">Review and verify payment submissions</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-neutral-100">{payments.length}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-neutral-800">
                  {payments.map((p, i) => (
                    <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-neutral-800/30 transition-colors">
                      <td className="px-5 py-4 text-xs font-mono text-neutral-300">{p.id}</td>
                      <td className="px-5 py-4 text-sm text-neutral-200">{p.bookingId || 'N/A'}</td>
                      <td className="px-5 py-4 text-sm text-neutral-400">{p.userName || 'N/A'}</td>
                      <td className="px-5 py-4 text-sm font-semibold text-neutral-200">₹{p.amount || 0}</td>
                      <td className="px-5 py-4 text-sm text-neutral-400">{p.method || 'whatsapp'}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1.5">
                          {p.screenshot && (
                            <a href={p.screenshot} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-neutral-800 hover:bg-neutral-700 rounded-lg transition-colors">
