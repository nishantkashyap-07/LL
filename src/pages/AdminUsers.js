      ...
      <div className="container-elegant py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-neutral-500 text-sm">Manage all platform users</p>
          </div>
        ...
        {/* Filters */}
        <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 mb-5 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            {['all', 'buyer', 'seller'].map((role) => (
              <button key={role} onClick={() => setFilterRole(role)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize ${filterRole === role ? 'bg-neutral-100 text-neutral-900' : 'bg-neutral-800 text-neutral-400 hover:text-neutral-200 border border-neutral-700'}`}>
                {role}
              </button>
            ))}
          </div>
              ...
              <thead className="bg-neutral-800/50 border-b border-neutral-800">
                <tr>
                  {['User', 'Role', 'Status', 'Bookings', 'Joined', 'Actions'].map((h) => (
                    <th key={h} className={`px-5 py-3.5 text-xs font-semibold text-neutral-400 ${h === 'Actions' ? 'text-right' : 'text-left'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-800">
                {filtered.map((u, i) => (
                  <motion.tr key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-neutral-800/30 transition-colors">
                    <td className="px-5 py-4">
                      <p className="text-sm font-medium text-neutral-200">{u.name || 'N/A'}</p>
                      <p className="text-xs text-neutral-500">{u.email}</p>
                    </td>
                    <td className="px-5 py-4">
                      <span className="px-2.5 py-1 bg-neutral-800 text-neutral-300 rounded-full text-xs font-medium capitalize">{u.isSeller ? 'seller' : 'buyer'}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium border capitalize ${getStatusStyle(u)}`}>{getStatusText(u)}</span>
                    </td>
                    <td className="px-5 py-4 text-sm text-neutral-300">{u.bookingsCount || 0}</td>
                    <td className="px-5 py-4 text-xs text-neutral-500">{formatDate(u.createdAt)}</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1.5">
                        <button className="p-1.5 bg-neutral-800 hover:bg-neutral-700 rounded-lg transition-colors"><Eye size={14} className="text-neutral-400" /></button>
