  ...
  if (!booking) return (
    <div className="pt-20 min-h-screen bg-neutral-950 flex items-center justify-center">
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-12 text-center max-w-sm">
        ...
        <h2 className="text-xl font-bold text-neutral-100 mb-2">Booking not found</h2>
        <p className="text-neutral-500 text-sm mb-6">We couldn't find the booking you're looking for.</p>
    ...
    <div className="pt-20 min-h-screen bg-neutral-950">
      ...
      <div className="container-elegant py-12">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center mb-10">
          ...
          <p className="text-neutral-400">Your vehicle rental has been confirmed. Get ready for an amazing journey!</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-neutral-800 rounded-xl mb-5">
                ...
                <div>
                  <p className="text-xs text-neutral-500 mb-0.5">{booking.vehicleType}</p>
                  <h3 className="text-base font-bold text-neutral-100">{booking.vehicleName}</h3>
                  <p className="text-xs text-neutral-500 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" />{booking.location}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.label}>
                    <p className="text-xs text-neutral-500 mb-1.5">{item.label}</p>
                    <div className={`px-3 py-2.5 bg-neutral-800 rounded-xl text-sm text-neutral-200 ${item.mono ? 'font-mono' : ''}`}>{item.value}</div>
                  </div>
                ))}
                <div>
                  <p className="text-xs text-neutral-500 mb-1.5">Pickup</p>
                  <div className="px-3 py-2.5 bg-neutral-800 rounded-xl">
                    <p className="text-sm text-neutral-200 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-neutral-400" />{booking.pickupDate}</p>
                ...
                <div>
                  ...
                  <div className="px-3 py-2.5 bg-neutral-800 rounded-xl">
                    <p className="text-sm text-neutral-200 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-neutral-400" />{booking.returnDate}</p>
              ...
              <div className="mt-5 p-4 bg-neutral-800 rounded-xl">
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-neutral-400"><span>Price per {booking.rentalType === 'hourly' ? 'hour' : 'day'}</span><span>₹{booking.pricePerDay || booking.pricePerHour}</span></div>
                  <div className="flex justify-between text-neutral-400"><span>Duration</span><span>{booking.duration || booking.days} {booking.rentalType === 'hourly' ? 'hours' : 'days'}</span></div>
                  <div className="flex justify-between pt-2 border-t border-neutral-700">
                    <span className="font-semibold text-neutral-200">Total Amount</span>
                    <span className="text-lg font-bold text-neutral-100">₹{booking.totalAmount}</span>
                  </div>
                </div>
            ...
            {/* Next Steps */}
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
              <h3 className="text-base font-semibold text-neutral-100 mb-5">What's Next?</h3>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5">
              <h3 className="text-sm font-semibold text-neutral-200 mb-4">Seller Contact</h3>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-neutral-700 rounded-full flex items-center justify-center text-neutral-200 font-bold text-sm">
                  {booking.seller?.name?.charAt(0)}
                </div>
                <div>
                  <p className="text-sm font-medium text-neutral-200">{booking.seller?.name}</p>
                  <p className="text-xs text-neutral-500">Vehicle Owner</p>
                </div>
              </div>
              <div className="space-y-2 mb-4">
                <a href={`tel:${booking.seller?.phone}`} className="flex items-center gap-2 p-2.5 bg-neutral-800 rounded-lg hover:bg-neutral-700 transition-colors">
                  <Phone className="w-3.5 h-3.5 text-neutral-400" /><span className="text-xs text-neutral-300">{booking.seller?.phone}</span>
                </a>
                <a href={`mailto:${booking.seller?.email}`} className="flex items-center gap-2 p-2.5 bg-neutral-800 rounded-lg hover:bg-neutral-700 transition-colors">
                  <Mail className="w-3.5 h-3.5 text-neutral-400" /><span className="text-xs text-neutral-300">{booking.seller?.email}</span>
                </a>
              </div>
              <button
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 space-y-2">
              <button onClick={() => toast.success('Receipt downloaded!')} className="w-full flex items-center justify-center gap-2 bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 text-neutral-300 py-2.5 rounded-xl text-sm transition-colors">
                <Download size={14} /> Download Receipt
              </button>
              <button onClick={handleShare} className="w-full flex items-center justify-center gap-2 bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 text-neutral-300 py-2.5 rounded-xl text-sm transition-colors">
