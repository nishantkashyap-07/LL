      ...
      <div className="container-elegant py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-neutral-500 text-sm">Monitor all platform bookings</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold text-neutral-100">{bookings.length}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-neutral-800">
                  {bookings.map((b, i) => (
                    <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-neutral-800/30 transition-colors">
                      <td className="px-5 py-4 text-xs font-mono text-neutral-300">{b.id}</td>
                      <td className="px-5 py-4 text-sm text-neutral-200">{b.vehicleName || 'N/A'}</td>
                      <td className="px-5 py-4 text-sm text-neutral-400">{b.userName || 'N/A'}</td>
                      <td className="px-5 py-4 text-sm text-neutral-400">{b.sellerName || 'N/A'}</td>
                      <td className="px-5 py-4 text-sm font-semibold text-neutral-200">₹{b.totalAmount || 0}</td>
