    ...
    <div className="pt-20 min-h-screen bg-neutral-950">
      ...
      <div className="container-elegant py-12 max-w-3xl">
        <div className="mb-8">
          ...
          <p className="text-neutral-500 text-sm">Manage your bank details and payout history</p>
        </div>

        {/* Bank Details */}
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 mb-5">
          <h2 className="text-base font-semibold text-neutral-100 mb-5">Bank Details</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
            {fields.map((f) => (
              <div key={f.key} className={f.full ? 'md:col-span-2' : ''}>
                <label className="block text-xs text-neutral-500 mb-1.5">{f.label}</label>
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
          <h2 className="text-base font-semibold text-neutral-100 mb-5">Payout History</h2>
              ...
              payouts.map((payout, i) => (
                <motion.div key={payout.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-neutral-200">₹{payout.amount}</p>
                      <p className="text-xs text-neutral-500">{payout.method} · {payout.date}</p>
                    </div>
                  </div>
