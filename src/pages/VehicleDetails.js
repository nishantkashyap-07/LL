    ...
    return (
      <div className="pt-20 min-h-screen bg-neutral-950 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-2 border-neutral-600 border-t-neutral-200 rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-neutral-500 text-sm">Loading vehicle details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="pt-20 min-h-screen bg-neutral-950 flex items-center justify-center">
        <div className="card-elegant text-center max-w-sm">
          ...
          <h2 className="text-xl font-bold text-neutral-100 mb-2">Vehicle not found</h2>
          <p className="text-neutral-400 text-sm">The vehicle you're looking for doesn't exist.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-20 min-h-screen bg-neutral-950">
      <div className="container-elegant py-10">
        {/* Back */}
        <button onClick={() => window.history.back()} className="inline-flex items-center gap-2 text-neutral-400 hover:text-neutral-200 text-sm mb-8 transition-colors group">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card-minimal">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h1 className="text-2xl font-bold text-neutral-100 mb-2">{vehicle.name}</h1>
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-1">
                      <Star className="w-4 h-4 text-secondary-400 fill-current" />
                      <span className="font-medium text-neutral-200 text-sm">{vehicle.rating}</span>
                      <span className="text-neutral-500 text-sm">({vehicle.reviews} reviews)</span>
                    </div>
                    {vehicle.location && (
                ...
                <div className="flex gap-2">
                  ...
                  <button className="p-2.5 bg-neutral-800 hover:bg-neutral-700 rounded-xl transition-colors text-neutral-400 hover:text-neutral-200">
                    <Share2 className="w-4 h-4" />
                  </button>
                </div>
              </div>

              <p className="text-neutral-400 text-sm leading-relaxed mb-6">{vehicle.description}</p>

              {/* Features */}
              {vehicle.features?.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-neutral-300 uppercase tracking-wider mb-3">Features</h3>
                  <div className="flex flex-wrap gap-2">
                    {vehicle.features.map((f, i) => (
                      <span key={i} className="px-3 py-1.5 bg-neutral-800 border border-neutral-700 text-neutral-300 rounded-lg text-xs font-medium">{f}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {vehicle.specs && (
                <div>
                  <h3 className="text-sm font-semibold text-neutral-300 uppercase tracking-wider mb-4">Specifications</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {Object.entries(vehicle.specs).map(([key, value]) => (
                      <div key={key} className="flex items-center gap-3 p-3 bg-neutral-800/50 rounded-xl">
                        <div className="w-8 h-8 bg-neutral-700 rounded-lg flex items-center justify-center flex-shrink-0">
                          {key === 'fuel' || key === 'mileage' || key === 'fuelCapacity' ? <Fuel className="w-4 h-4 text-neutral-400" /> : key === 'seats' ? <Users className="w-4 h-4 text-neutral-400" /> : <Settings className="w-4 h-4 text-neutral-400" />}
                        </div>
                        <div>
            ...
            {vehicle.included?.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card-minimal">
                <h3 className="text-sm font-semibold text-neutral-300 uppercase tracking-wider mb-4">What's Included</h3>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-minimal">
              <h3 className="text-sm font-semibold text-neutral-300 uppercase tracking-wider mb-4">
                Reviews {reviews.length > 0 && <span className="text-neutral-500 normal-case font-normal">({reviews.length})</span>}
              </h3>
              ...
              {reviews.length > 0 ? (
                <div className="space-y-4 mb-6">
                  {reviews.map(r => (
                    <div key={r.id} className="p-4 bg-neutral-800/40 rounded-xl border border-neutral-700/40">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-neutral-200">{r.userName || 'Anonymous'}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          {[1,2,3,4,5].map(s => (
                      ...
                      <p className="text-neutral-400 text-sm">{r.comment}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-neutral-500 text-sm mb-6">No reviews yet. Be the first to review!</p>
              )}

              {/* Submit review form */}
              {user && (
                <div className="border-t border-neutral-800 pt-4">
                  <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-3">Leave a Review</p>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2.5 bg-neutral-800 border border-neutral-700 rounded-xl text-sm text-neutral-200 placeholder:text-neutral-500 focus:outline-none focus:border-neutral-500 resize-none mb-3"
                  />
                  <button onClick={handleSubmitReview} disabled={submittingReview}
                    className="flex items-center gap-2 px-4 py-2 bg-neutral-700 hover:bg-neutral-600 text-neutral-200 rounded-xl text-sm font-medium transition-colors disabled:opacity-50">
          ...
          <div className="lg:col-span-1">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="card-minimal sticky top-24">
              <div className="text-center mb-6 pb-6 border-b border-neutral-800">
                <div className="text-3xl font-bold text-neutral-100">₹{rentalType === 'hourly' ? (vehicle.pricePerHour || Math.round(vehicle.price / 8)) : vehicle.price}</div>
                <div className="text-neutral-500 text-sm">per {rentalType === 'hourly' ? 'hour' : 'day'}</div>
              </div>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Pickup Date</label>
                ...
                {rentalType === 'hourly' && (
                  <div>
                    <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Pickup Time</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Return Date</label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 w-4 h-4" />
                ...
                {rentalType === 'hourly' && (
                  <div>
                    <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-2">Return Time</label>
              ...
              {duration > 0 && (
                <div className="bg-neutral-800/50 rounded-xl p-4 mb-6 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between font-semibold text-neutral-100 pt-2 border-t border-neutral-700">
              ...
              <button onClick={handleBooking} className="w-full flex items-center justify-center gap-2 bg-neutral-100 hover:bg-white text-neutral-900 font-semibold py-4 rounded-xl transition-all mb-3 text-sm">
                <MessageCircle className="w-4 h-4" />
                Book via WhatsApp
              </button>
              <p className="text-center text-xs text-neutral-500 mb-6">Secure payment via WhatsApp · No upfront charges</p>
              {/* Owner */}
              {vehicle.owner && (
                <div className="pt-6 border-t border-neutral-800">
                  <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-3">Hosted by</p>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-neutral-200 text-sm">{vehicle.owner.name}</p>
              ...
              <div className="flex items-center gap-2 mt-4 pt-4 border-t border-neutral-800 text-xs text-neutral-500">
