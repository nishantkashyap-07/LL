  ...
  if (!application) return (
    <div className="pt-20 min-h-screen bg-neutral-50 dark:bg-neutral-950 flex items-center justify-center">
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-12 text-center max-w-sm">
        ...
        <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100 mb-2">No Application Found</h2>
        <p className="text-neutral-500 dark:text-neutral-500 text-sm mb-6">You haven't submitted a seller application yet.</p>
    ...
    <div className="pt-20 min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <div className="container-elegant py-12 max-w-3xl">
        <div className="mb-8">
          ...
          <p className="text-neutral-500 dark:text-neutral-500 text-sm">Track your seller application progress</p>
        </div>

        {/* Status Card */}
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className={`bg-white dark:bg-neutral-900 border rounded-2xl p-8 text-center mb-5 ${cfg.color}`}>
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${cfg.color}`}>
            <StatusIcon className="w-8 h-8" />
          </div>
          <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100 mb-2">{cfg.title}</h2>
          <p className="text-neutral-600 dark:text-neutral-400 text-sm max-w-md mx-auto">{cfg.message}</p>
        </motion.div>

        {/* Timeline */}
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 mb-5">
          <h3 className="text-base font-semibold text-neutral-900 dark:text-neutral-100 mb-5">Application Timeline</h3>
          <div className="space-y-4">
            {timeline.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-start gap-4">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${item.done ? 'bg-emerald-500/20 border border-emerald-500/30' : 'bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700'}`}>
                  {item.done ? <CheckCircle className="w-4 h-4 text-emerald-400" /> : <Clock className="w-4 h-4 text-neutral-500 dark:text-neutral-500" />}
                </div>
                <div>
                  <p className={`text-sm font-medium ${item.done ? 'text-neutral-800 dark:text-neutral-200' : 'text-neutral-500 dark:text-neutral-500'}`}>{item.step}</p>
        ...
        {/* Documents */}
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-6 mb-5">
          <h3 className="text-base font-semibold text-neutral-900 dark:text-neutral-100 mb-4">Submitted Documents</h3>
          <div className="grid grid-cols-3 gap-3">
            {['Aadhaar Card', 'PAN Card', 'Driving License'].map((doc) => (
              <div key={doc} className="flex items-center justify-between p-3 bg-neutral-100 dark:bg-neutral-800 rounded-xl">
        ...
        {status === 'approved' && (
          <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-5 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200 mb-0.5">Ready to Start Selling?</p>
              <p className="text-xs text-neutral-500 dark:text-neutral-500">List your first vehicle and start earning today!</p>
            </div>
        ...
        {status === 'rejected' && (
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5">
            <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200 mb-2">Rejection Reason</p>
            <p className="text-xs text-neutral-500 dark:text-neutral-500 mb-4">Your documents could not be verified. Please ensure all information is accurate and documents are clear.</p>
            <Link to="/seller/apply">
              <button className="flex items-center gap-2 bg-neutral-100 hover:bg-white text-neutral-900 font-semibold px-5 py-2.5 rounded-xl text-sm transition-all">
